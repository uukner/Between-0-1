...
class Window(Frame):
    ...
    def init_window(self):
        self.master.title('Between 0 and 1')
        ...
        self.frame_info.place(x=-300,y=0)
        ...
        self.frame_pad = ttk.LabelFrame(self.master, text="Writing area")
        self.frame_pad.place(x=10, y=10, width=400, height=400)
        
        # 创建画布
        image = tkinter.PhotoImage()
        self.canvas = tkinter.Canvas(self.frame_pad, bg='white', width=400, height=400)
        self.canvas.create_image(240, 240, image=image)
        ...
        self.base = Image.new("RGB", (400, 400), (255,255,255))
        self.d = ImageDraw.Draw(self.base)
        
       
        button_start = ttk.Button(text="Start", width=10, command=self.load_model)
        button_start.place(x=76, y=440, anchor='center')
        button_cl = ttk.Button(text="Clear", width=10, command=self.Clear)        
        button_cl.place(x=273, y=440, anchor='center')
        button_reg = ttk.Button(text="Recognize", width=10, command=self.predict)
        button_reg.place(x=470, y=440, anchor='center')  
        
        self.frame2 = ttk.LabelFrame(self.master, text="Recognition Result")
        self.frame2.place(x=420, y=10, width=300, height=340)
        image2 = tkinter.PhotoImage()
        self.canvas2 = tkinter.Canvas(self.frame2, bg='black', width=400, height=400)
        self.canvas2.create_image(240, 240, image=image2)
        self.canvas2.pack(fill=tkinter.BOTH, expand=tkinter.YES)
        
        
        self.label = ttk.Label(self.master, text="Enter number as\nsample label",anchor="center",font=("Helvatica",15))
        self.label.place(x=535,y=358,width=180,height=50)
        self.numEntry = Entry(self.master, font=("Cream_12",30))
        self.numEntry.place(x=420,y=358,width=120,height=50)
        button_save = ttk.Button(self.master, text="Save", command=self.saveSample)
        button_save.place(x=600,y=425.5,width=120,height=30)
    ...
    def Clear(self):
        # pillow的img对象重新画成白色的
        self.d.rectangle([0,0,400,400],fill='white')
        ...
        self.label['text'] = "Enter number as\nsample label"
    ...
    def trainning(self, newSample=None):
        ...
        for i in range(len(index)):
            x_list.append(X[index[i]])
            y_list.append(y[index[i]])
        ...
        nm = NeuralNetwork([64, 100, 50, 2])#创建网络

        X_train, X_test, y_train, y_test = train_test_split(X,y)
        # labels_train = LabelBinarizer().fit_transform(y_train)
        # labels_test = LabelBinarizer().fit_transform(y_test)
        labels_train = np.eye(2)[y_train].astype(np.int16)
        labels_test = np.eye(2)[y_test].astype(np.int16)
    ...
    def predict(self):
        ...
        try:
            ...
            print("The probability of being recognized as a digit 0 is: {}".format(result[0][0]))
            print("The probability of being recognized as a digit 1 is: {}".format(result[0][1]))

        except AttributeError:
            self.canvas2.create_text(18, 20,
            text = 'Please click Start first',
            font = ("Cream_12", 16, "bold"),
            fill= "red",
            anchor = W,
            justify = LEFT)
        ...
        titleFont = ("Cream_12", 220, "bold")
        self.canvas2.create_text(100, 100,
        ...
        textFont1 = ("Helvatica", 12)
        textFont2 = ("Cream_12", 25)
        
        self.canvas2.create_text(20, 210,
            text = "The probability of being recognized as 0 is: ",
            font = textFont1,
            fill= "White",
            anchor = W,
            justify = LEFT)
        
        self.canvas2.create_text(20, 230,
            text = format(result[0][0]),
            font = textFont2,
            fill= "White",
            anchor = W,
            justify = LEFT)
        
        self.canvas2.create_text(20, 270,
            text = "The probability of being recognized as 1 is: ",
            font = textFont1,
            fill= "White",
            anchor = W,
            justify = LEFT)
        ...
        self.canvas2.create_text(20, 290,
            text = format(result[0][1]),
            font = textFont2,
            fill= "White",
            anchor = W,
            justify = LEFT)
    ...
    def pre_job(self):
        ...
        if r_cs.max() < 2:
            ...
            text = 'Please write the number in \nthe writing area first',
            font = ("Cream_12", 18, "bold"),
            fill= "red",
            anchor = W,
            justify = LEFT)
